...
class tx_abdownloads_pi1 extends AbstractPlugin
{
    ...
    public $extKey = 'ab_downloads';                    // The extension key.
    public $tablePrefix = 'tx_abdownloads_';                // The database table prefix.
    public $originalTemplateCode = null;                // Holds template code.
    public $cObj;                            // Reference to the calling cObj.
    public $markerBasedTemplateService = null;
    public $debug = false;                        // Global debug switch. Change to 'true' for debugging information.
    public $debugDB = false;                        // Database debug switch. Change to 'true' for debugging information.
    public $full_debug = false;                    // Full debug switch. Change to 'true' for full debugging information.
    public $sysfolderList = null;                    // Holds the 'Starting Point' PIDs.
    public $filePath = null;                        // Holds the file path for downloads.
    public $flexform = null;                        // Holds the flexform configuration for the plugin.
    public $pi_checkCHash = true;                    // Enable cHash check.
    public $allowCaching;                        // Holds cache setting.
    public $sys_language_mode;                        // Holds the language mode.
    public $alternatingLayouts;                    // Holds the number of alternating layouts.
     ...
     */
    public function displayCategory($categoryUID = 0)
    {
        ...
        // Create marker array
        $markerArray = [];
        $markerArray['###CATEGORY_PATH###'] = $this->local_cObj->stdWrap(
            $this->getCategoryPath($categoryUID),
            $localConf['categoryPath_stdWrap.']
        );

        // Substitute the markers in the given sub sub part
        $subpartContent = null;
        $subpartContent = $this->markerBasedTemplateService->substituteMarkerArray($this->markerBasedTemplateService->getSubpart(
            $templateCode,
            '###' . $subSub_pathmenu . '###'
        ), $markerArray);

        // Substitute the template code with the given subpartcontent
        $templateCode = $this->markerBasedTemplateService->substituteSubpart(
            $templateCode,
            '###' . $subSub_pathmenu . '###',
            $subpartContent
        );
        ...
        $allowAddDownloads = $this->pi_getFFvalue(
            $this->flexform,
            'allowAddDownloads',
            'sDEF'
        ) ? $this->pi_getFFvalue(
        ...
        if ($isLoggedIn == 1 || $allowAddDownloads == 1) {
            ...
            $markerArray['###DOWNLOAD_ADD_NEW###'] = $this->local_cObj->stdWrap(
                $download,
        ...
        if ($categoryUID == 0) {
            ...
            $orderBy = '';
            $limit = '';
            $downloadsInCurrentCategoryResults = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                '*',
                $databaseTable,
                $whereClause,
                $groupBy,
            ...
            $orderBy = '';
            $limit = '';
            $downloadsInCurrentCategoryResults = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
                $databaseTable . '.*',
                $databaseTable,
                $relationTable,
                $foreignTable,
                ' AND ' . $theField . '=' . $GLOBALS['TYPO3_DB']->fullQuoteStr(
                    $theValue,
                    $databaseTable
                ) . ' ' . $whereClause,
                $groupBy,
        ...
        // Get record overlay
        $downloadsInCurrentCategory = $this->getRecordOverlay(
            $downloadsInCurrentCategoryResults,
        ...
        // Get category record(s)
        $categoryLabelsResults = $this->getCategoryRecords(
            'parent_category',
            $categoryUID,
        ...
        if (is_array($categoryLabels) && count($categoryLabels) > 0) {
            ...
            for ($i = 0; $i < count($categoryLabels); $i++) {
                ...
                $markerArrayCategory['###CATEGORY_ICON###'] = $this->getImageLink(
                    $categoryLabels[$i],
                    'image',
                    'category',
                    'list',
                ...
                $markerArrayCategory['###CATEGORY_LABEL###'] = $this->local_cObj->stdWrap(
                    $download,
                    $localConf['categoryLabel_stdWrap.']
                );
                $markerArrayCategory['###CATEGORY_DOWNLOAD_COUNT###'] = $this->local_cObj->stdWrap(
                    $categoryDownloadCount,
                    ''
                );
                $markerArrayCategory['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap(
                    htmlspecialchars(trim($this->pi_getLL('ll_downloads_lower'))),
                    ''
                );
                $markerArrayCategory['###LL_TOTAL###'] = $this->local_cObj->stdWrap(
                    htmlspecialchars(trim($this->pi_getLL('ll_total'))),
                    ''
                );

                if ($categoryLabels[$i]['description']) {
                    $markerArrayCategory['###CATEGORY_DESCRIPTION###'] = $this->pi_RTEcssText($this->local_cObj->stdWrap(
                        trim($categoryLabels[$i]['description']),
                ...
                // Substitute the markers in the given sub sub part
                $categoryList .= $this->markerBasedTemplateService->substituteMarkerArray($this->markerBasedTemplateService->getSubpart(
                    $templateCode,
                    '###' . $subSubSub_category . '###'
                ), $markerArrayCategory);
            }
            ...
            // Prepare title array
            $markerArrayTitle = [];
            $markerArrayTitle['###LL_CATS###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_cats'))),
                $localConf['categories_stdWrap.']
            );
            $markerArrayTitle['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_downloads_lower'))),
                $localConf['downloads_stdWrap.']
            );
            $markerArrayTitle['###LL_TOTAL###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_total'))),
                ''
            );
            $markerArrayTitle['###DOWNLOAD_COUNT_TOTAL###'] = $this->local_cObj->stdWrap(
                $categoryDownloadCountTotal + count($downloadsInCurrentCategory),
                ''
            );

            $wrappedSubpartArray = [];
            $subpartArray['###CATEGORY###'] = $categoryList;
            $subpartContent = $this->markerBasedTemplateService->substituteMarkerArrayCached($this->markerBasedTemplateService->getSubpart(
                $templateCode,
                '###' . $subSub_categories . '###'
            ), $markerArrayTitle, $subpartArray, $wrappedSubpartArray);
        ...
        $listLimit = intval($this->pi_getFFvalue(
            $this->flexform,
            'listLimit',
            's_display'
        ) ? $this->pi_getFFvalue($this->flexform, 'listLimit', 's_display') : $this->conf['listLimit']);
        ...
        if ($categoryUID == 0) {
            ...
            $downloadsResults = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                '*',
                $databaseTable,
                $whereClause,
                $groupBy,
            ...
            $orderBy = $downloadSortBy . ' ' . $downloadSortOrder;
            $limit = $limitStart . ', ' . $listLimit;
            $downloadsResults = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
                $databaseTable . '.*',
                $databaseTable,
                $relationTable,
                $foreignTable,
        ...
        if (is_array($downloads) && count($downloads) > 0) {
            ...
            $markerArrayTitle = [];
            $markerArrayTitle['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_downloads'))),
                $localConf['downloads_stdWrap.']
            );
        ...
        if (count($downloadsInCurrentCategory) > $listLimit && !$this->conf['noPageBrowser'] && !$this->pi_getFFvalue(
            $this->flexform,
            'noPageBrowser',
            's_display'
        )) {
            ...
            if ($this->conf['userPageBrowserFunc']) {
                ...
                if ($this->conf['usePiBasePagebrowser']) {
                    ...
                    $markerArray['###BROWSE_DOWNLOADS###'] = $this->pi_list_browseresults(
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function displayTree($categoryUID = 0, $level = 0)
    {
        ...
        if (is_array($downloads) && count($downloads) > 0) {
            for ($j = 0; $j < count($downloads); $j++) {
                ...
                // Substitute the markers in the given sub sub part
                $downloadList .= $this->markerBasedTemplateService->substituteMarkerArray($this->markerBasedTemplateService->getSubpart(
                    $templateCode,
                    '###' . $subSub_treedownload . '###'
                ), $markerArray);
            }
            ...
            $markerArrayMessage['###CATEGORY_DOWNLOAD_COUNT###'] = $this->local_cObj->stdWrap(
                $categoryDownloadCount,
                ''
            );
            $markerArrayMessage['###INDENTION###'] = $indention;
            $markerArrayMessage['###LEVEL###'] = $level;
            $markerArrayMessage['###CATEGORY_ICON###'] = $this->getImageLink(
                $category[0],
                'image',
                'category',
                'tree',
            ...
            $markerArrayMessage['###CATEGORY_LABEL###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($category[0]['label'])),
                $localConf['categoryLabel_stdWrap.']
            );
            ...
            if ($isLoggedIn == 1 || $allowAddDownloads == 1) {
                ...
                $markerArrayMessage['###DOWNLOAD_ADD_NEW###'] = $this->local_cObj->stdWrap(
                    $download,
                    $localConf['downloadAddNew_stdWrap.']
                );
                $GLOBALS['TSFE']->ATagParams = $originalATagParams;
            ...
            if ($category[0]['description']) {
                $markerArrayMessage['###CATEGORY_DESCRIPTION###'] = $this->pi_RTEcssText($this->local_cObj->stdWrap(
                    trim($category[0]['description']),
                    $localConf['categoryDescription_stdWrap.']
                ));
            } else {
        ...
        // Get subcategories
        $subcategoriesResults = $this->getCategoryRecords(
            'parent_category',
            $categoryUID,
            $categorySortBy . ' ' . $categorySortOrder
        );
    ...
    public function displaySearch()
    {
        ...
        $formContent = $this->markerBasedTemplateService->substituteMarkerArray($this->markerBasedTemplateService->getSubpart(
            $templateCode,
            '###' . $subSub_form . '###'
        ), $markerArrayForm);
        ...
        if ($searchWords != null) {
            ...
            for ($i = 0; $i < count($downloads); $i++) {
                ...
                $markerArray['###CATEGORY_ICON###'] = $this->getImageLink(
                    $category[0],
                    'image',
                    'category',
                    'search',
                ...
                $markerArray['###CATEGORY_LABEL###'] = $this->local_cObj->stdWrap(
                    $download,
                    $localConf['categoryLabel_stdWrap.']
                );

                if ($category[0]['description']) {
                    $markerArray['###CATEGORY_DESCRIPTION###'] = $this->pi_RTEcssText($this->local_cObj->stdWrap(
                        trim($category[0]['description']),
                        $localConf['categoryDescription_stdWrap.']
                    ));
                } else {
                ...
                $pageID = intval($this->pi_getFFvalue(
                    $this->flexform,
                    'pageListPlugin',
                    'sDEF'
                ) ? $this->pi_getFFvalue($this->flexform, 'pageListPlugin', 'sDEF') : $GLOBALS['TSFE']->id);
            ...
            if (count($downloads) != 0) {
                ...
                $markerArray = [];
                $markerArray['###RESULT_MESSAGE###'] = sprintf($this->local_cObj->stdWrap(
                    $this->pi_getLL('ll_result'),
                    $localConf['resultMessage_stdWrap.']
                ), $searchFor, count($downloads));
            } else {
                // Create marker array
                $markerArray = [];
                $markerArray['###RESULT_MESSAGE###'] = '';
                $markerArray['###NO_DOWNLOADS_MESSAGE###'] = $this->local_cObj->stdWrap(
                    htmlspecialchars(trim($this->pi_getLL('no_downloads_message'))),
        ...
        $markerArray['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_downloads'))),
            $localConf['downloads_stdWrap.']
        );
        ...
        if (count($downloads) > $listLimit && !$this->conf['noPageBrowser'] && !$this->pi_getFFvalue(
            $this->flexform,
            'noPageBrowser',
            's_display'
        )) {
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function displayTop($categoryUID = 0)
    {
...
*/
                    ...
                    $topDownloadsResults = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
                        $selectExtension . $databaseTable . '.*',
                        $databaseTable,
                        $relationTable,
                        $foreignTable,
                        ' AND ' . $whereClause,
                    ...
                    $fieldName = $this->local_cObj->stdWrap(
                        htmlspecialchars(trim($this->pi_getLL('ll_' . $field))),
                        ''
                    );
                    ...
                    for ($i = 0; $i < count($topDownloads); $i++) {
                        ...
                        $markerArrayDownload['###CATEGORY_PATH###'] = $this->local_cObj->stdWrap(
                            $this->getCategoryPath($categoryUID),
                            $localConf['categoryPath_stdWrap.']
                        );
                        $markerArrayDownload['###CATEGORY_ICON###'] = $this->getImageLink(
                            $topDownloads[$i],
                            'image',
                            'category',
                            'top',
                        ...
                        $markerArrayDownload['###CATEGORY_LABEL###'] = $this->local_cObj->stdWrap(
                            $download,
                            $localConf['categoryLabel_stdWrap.']
                        );

                        if ($topDownloads[$i]['description']) {
                            $markerArrayDownload['###CATEGORY_DESCRIPTION###'] = $this->pi_RTEcssText($this->local_cObj->stdWrap(
                                trim($topDownloads[$i]['description']),
                                $localConf['categoryDescription_stdWrap.']
                            ));
                        } else {
                    ...
                    if (count($topDownloads) > 0) {
                        // Prepare top message
                        $markerArrayMessage = [];
                        $markerArrayMessage['###TOP_MESSAGE###'] = $this->local_cObj->stdWrap(
                            htmlspecialchars(trim($this->pi_getLL('ll_top') . $downloadLimit . ': ' . $fieldName)),
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function displayCatalog($categoryUID = 0, $level = 0)
    {
        ...
        $categorySortBy = $this->pi_getFFvalue($this->flexform, 'categorySortBy', 's_display');
        $categorySortOrder = $this->pi_getFFvalue($this->flexform, 'categorySortOrder', 's_display');
        $numberOfColumns = intval($this->pi_getFFvalue(
            $this->flexform,
            'numColumns',
            's_catalog'
        ) ? $this->pi_getFFvalue($this->flexform, 'numColumns', 's_catalog') : 2);
        $maxNumSubcat = intval($this->pi_getFFvalue(
            $this->flexform,
            'maxSubcategories',
            's_catalog'
        ) ? $this->pi_getFFvalue($this->flexform, 'maxSubcategories', 's_catalog') : 2);
        ...
        if (is_array($categoryLabels) && count($categoryLabels) > 0) {
            ...
            for ($i = 0; $i < count($categoryLabels); $i++) {
                ...
                // Get subcategories
                $subcategoryLabelsResults = $this->getCategoryRecords(
                    'parent_category',
                    $categoryLabels[$i]['uid'],
                    $categorySortBy . ' ' . $categorySortOrder,
                    $maxNumSubcat
                );

                // Get record overlay
                $subcategoryLabels = $this->getRecordOverlay(
                    $subcategoryLabelsResults,
                ...
                if (count($subcategoryLabels) > 0) {
                    for ($j = 0; $j < count($subcategoryLabels); $j++) {
                        ...
                        $markerArraySubcategory['###SUBCATEGORY_ICON###'] = $this->getImageLink(
                            $subcategoryLabels[$j],
                            'image',
                            'category',
                            'catalog',
                        ...
                        $markerArraySubcategory['###SUBCATEGORY_LABEL###'] = $this->local_cObj->stdWrap(
                            $download,
                            $localConf['subcategoryLabel_stdWrap.']
                        );
                        $markerArraySubcategory['###SUBCATEGORY_DOWNLOAD_COUNT###'] = $this->local_cObj->stdWrap(
                            $subcategoryDownloadCount,
                            ''
                        );
                        $markerArraySubcategory['###DOWNLOAD_COUNT_TOTAL###'] = $this->local_cObj->stdWrap(
                            $subcategoryLinkCountTotal + count($linksInCurrentCategory),
                            ''
                        );
                        $markerArraySubcategory['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap(
                            htmlspecialchars(trim($this->pi_getLL('ll_downloads_lower'))),
                            ''
                        );
                        $markerArraySubcategory['###LL_TOTAL###'] = $this->local_cObj->stdWrap(
                            htmlspecialchars(trim($this->pi_getLL('ll_total'))),
                            ''
                        );

                        if ($subcategoryLabels[$j]['description']) {
                            $markerArraySubcategory['###SUBCATEGORY_DESCRIPTION###'] = $this->pi_RTEcssText($this->local_cObj->stdWrap(
                                trim($subcategoryLabels[$j]['description']),
                        ...
                        // Substitute the markers in the given sub sub part
                        $subcategoryList .= $this->markerBasedTemplateService->substituteMarkerArray($this->markerBasedTemplateService->getSubpart(
                            $templateCode,
                            '###' . $subSubSub_subcategories . '###'
                        ), $markerArraySubcategory) . $separator;
                    }
                }

                $markerArrayCategory['###DOWNLOAD_COUNT_TOTAL###'] = $this->local_cObj->stdWrap(
                    $subcategoryDownloadCountTotal + count($downloadsInCurrentCategory),
                    ''
                );
                ...
                $subpartContent .= $this->markerBasedTemplateService->substituteMarkerArrayCached($this->markerBasedTemplateService->getSubpart(
                    $templateCode,
                    '###' . $subSub_category . '###'
                ), $markerArrayCategory, $subpartArray, $wrappedSubpartArray);
            }
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function getViewClickedDownload($uid = null)
    {
        if ($uid != null) {
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function getViewDetailsForDownload($uid = null, $categoryUID)
    {
...
//                    $fileInformation = $this->getTotalFileInfo($file);
                ...
                $markerArray['###DOWNLOAD_IMAGE###'] = $this->getImageLink(
                    $download[0],
                    'image',
                    'link',
                    'list',
                ...
                $templateCode = $this->local_cObj->stdWrap(
                    htmlspecialchars(trim($this->pi_getLL('non_public_download_message'))),
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function getViewAddNewDownload($categoryUID, $form_errormsg = null)
    {
        ...
        $maxFileSize = $this->pi_getFFvalue(
            $this->flexform,
            'maxFileSize',
            'sDEF'
        ) ? $this->pi_getFFvalue($this->flexform, 'maxFileSize', 'sDEF') : $this->conf['maxFileSize'];
        ...
        if (is_object($this->freeCap)) {
            ...
            $markerArray['###SR_FREECAP_NOTICE###'] = $this->local_cObj->stdWrap(
            ...
            $markerArray['###CAPTCHA_NOTICE###'] = $this->local_cObj->stdWrap(
        ...
        $markerArray['###FORM_ERRORMSG###'] = $this->local_cObj->stdWrap(
            $form_errormsg,
        ...
        $markerArray['###LL_ADD_DOWNLOAD###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_add_download'))),
            $this->conf['addDownload_stdWrap.']
        );
        $markerArray['###LL_ADD_DOWNLOAD_TEXT###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_add_download_text'))),
            $this->conf['addDownloadText_stdWrap.']
        );
        $markerArray['###LL_LABEL_EXAMPLE###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_label_example'))),
            ''
        );
        $markerArray['###LL_CONTACT_EXAMPLE###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_contact_example'))),
            ''
        );
        $markerArray['###LL_IMAGE_EXAMPLE###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_image_example'))),
            ''
        );
        $markerArray['###LL_MAXIMUM###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_maximum'))),
            ''
        );
        ...
        foreach ($possibleFields as $id => $possibleField) {
            ...
            $markerArray['###LL_' . strtoupper($possibleField) . '###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_' . $possibleField))),
                $wrap
            );
            $wrap = '';
        }
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function getViewAddNewDownloadResult($categoryUID = 0)
    {
        ...
        if ($statusAddedDownloads == 1) {
            $markerArray['###LL_LI_ADD_THX###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_appr_li_add_thx'))),
                ''
            );
        } else {
            $markerArray['###LL_LI_ADD_THX###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_li_add_thx'))),
                ''
            );
        }
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function getViewReportBrokenDownload($uid = null, $categoryUID)
    {
        ...
        if ($uid != null) {
            ...
            $markerArray['###DOWNLOAD_LABEL###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($download[0]['label'])),
                ''
            );
            $markerArray['###LINK_BACK_TO_CATEGORY###'] = '<a href="javascript:history.back()">' . htmlspecialchars(trim($this->pi_getLL('ll_back'))) . '</a>';
            ...
            $markerArray['###LL_REPORT_DOWNLOAD_BROKEN_TEXT###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_report_download_broken_text'))),
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function getViewReportBrokenDownloadResult($uid = null, $categoryUID)
    {
        ...
        if ($uid != null) {
            ...
            if ($download[0]['status'] == 1) {
                ...
                $markerArray['###LINK_BACK_TO_CATEGORY###'] = '<a href="javascript:history.go(-2)">' . htmlspecialchars(trim($this->pi_getLL('ll_back'))) . '</a>';
                $markerArray['###LL_LI_BROKEN_THX###'] = $this->local_cObj->stdWrap(
                    htmlspecialchars(trim($this->pi_getLL('ll_li_broken_thx'))),
                    ''
                );
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function getViewRateDownload($uid = null, $categoryUID)
    {
        ...
        if ($uid != null) {
            ...
            $markerArray['###LL_RATE_DOWNLOAD_TEXT###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_rate_download_text'))),
                $this->conf['rateDownloadText_stdWrap.']
            );
            $markerArray['###LL_VERY_BAD###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_very_bad'))),
                ''
            );
            $markerArray['###LL_VERY_GOOD###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_very_good'))),
                ''
            );
            $markerArray['###LL_RATING_CURRENT###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_rating_current'))),
                ''
            );
            $markerArray['###LL_VOTES###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_votes'))),
                ''
            );
     ...
     * @return    string        The generated HTML source for this view.
     */
    public function getViewRateDownloadResult($uid = null, $categoryUID)
    {
        // Init some vars
        $content = null;
        $templateCode = null;       // Hold the template source code
        ...
        if ($uid != null) {
            ...
            $markerArray['###LINK_BACK_TO_CATEGORY###'] = '<a href="javascript:history.go(-2)">' . htmlspecialchars(trim($this->pi_getLL('ll_back'))) . '</a>';
            $markerArray['###LL_LI_RATE_THX###'] = $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_li_rate_thx'))),
                ''
            );
     ...
     */
    public function makePageBrowser($pointerName = 'pointer')
    {
        ...
        if ($this->pi_alwaysPrev >= 0) {
            if ($pointer > 0) {
                $links[] = '<td nowrap="nowrap"><p>' . $this->pi_linkTP_keepPIvars(htmlspecialchars(trim($this->pi_getLL(
                ...
                $links[] = '<td nowrap="nowrap"><p>' . htmlspecialchars(trim($this->pi_getLL(
                    'pi_list_browseresults_prev',
                    '< Previous'
                ))) . '</p></td>';
            }
        ...
        for ($a = 0; $a < $max; $a++) {
            $links[] = '<td' . ($pointer == $a ? $this->pi_classParam('browsebox-SCell') : '') . ' nowrap="nowrap"><p>' . $this->pi_linkTP_keepPIvars(trim(htmlspecialchars(trim($this->pi_getLL(
     ...
     */
    public function userProcess($configKey, $variable)
    {
    ...
    public function initPidList()
    {
        ...
        $pidList = $pidList ? $pidList : trim($this->cObj->stdWrap(
        ...
        $recursive = is_numeric($recursive) ? $recursive : $this->cObj->stdWrap(
            $this->conf['recursive'],
            $this->conf['recursive.']
        );
     ...
     */
    public function recursiveDownloadCount($categoryUID)
    {
     ...
     */
    public function recursiveCategoryGet($categoryUID)
    {
     ...
     */
    public function getCategoryPath($categoryUID, $showLinks = true)
    {
        ...
        if ($categoryUID != '0') {
                    ...
                    if ($this->downloadMode == 'TREE') {
                        $conf = [
                            ...
                            'additionalParams' => GeneralUtility::implodeArrayForUrl(
                                '',
                                ['tx_abdownloads_pi1[category_uid]' => ''],
                                '',
                                0
                            ),
                        ];
            ...
            if ($showLinks) {
                $content = $this->pi_LinkTP(
                    htmlspecialchars(trim($this->pi_getLL('ll_home'))),
                    ['tx_abdownloads_pi1[cid]' => $this->cObj->data['uid']],
                    $this->allowCaching,
                    $pageID
                ) . $content;
     ...
     */
    public function getRecordOverlay(&$resultSet, $databaseTable)
    {
     ...
     */
    public function getCategorySelect($categoryUID, $selectedID = 0)
    {
        ...
        if ($categoryUID != '0') {
            ...
            $content .= '<option value="' . $categoryUID . '">' . $this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_cats'))),
                ''
            ) . '</option>';
        }
     ...
     */
    public function getDownloadRecord($downloadUID)
    {
        ...
        $orderBy = '';
        $limit = '';
        $downloadResult = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
            '*',
            $databaseTable,
            $theField . '=' . $GLOBALS['TYPO3_DB']->quoteStr($theValue, $databaseTable) . ' ' . $whereClause,
            $groupBy,
            $orderBy,
            $limit
        );
     ...
     */
    public function getCategoryRecords($field, $categoryUID, $orderBy = '', $limit = '')
    {
        ...
        $categoryResult = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
            '*',
            $databaseTable,
            $field . '=' . $GLOBALS['TYPO3_DB']->quoteStr($theValue, $databaseTable) . ' ' . $whereClause,
            $groupBy,
            $orderBy,
            $limit
        );
     ...
     */
    public function getCategoryUID($recordUID)
    {
        ...
        $orderBy = 'sorting ASC';
        $limit = '';
        $categoriesResult = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
            $foreignTable . '.*',
            $databaseTable,
            $relationTable,
            $foreignTable,
     ...
     */
    public function displayFEHelp($type)
    {
        switch ((string)$type) {
            case 'noTopListSet':
     ...
     */
    public function containsBlacklistedWords($text)
    {
     ...
     */
    public function checkInputFields($categoryUID)
    {
        $mandatoryFields = $this->pi_getFFvalue($this->flexform, 'mandatoryFields', 'sDEF');
        $errormsg = '';
     ...
     */
    public function getImageLink($record, $field, $type, $view, $categoryUID, $detailedView = false)
    {
                ...
                if ($record[$field]) {
                    ...
                    if ($linkImages != 'none') {
                        if ($linkImages == 'direct') {
                            $linkImage = $this->pi_LinkTP($this->local_cObj->cObjGetSingle(
                                'IMAGE',
                                $pictureConfig['image.']
                            ), [
                            ...
                            $pictureConfig['image.'] = array_merge(
                                $pictureConfig['image.'],
     ...
     */
    public function getFileIcon($record)
    {
     ...
     */
    public function getLanguage($language_uid)
    {
        if ($language_uid != '0') {
     ...
     */
    public function returnStarsForRating($rating)
    {
        $string = null;
        $count = 0;

        $iconText = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_rating'))),
            ''
        ) . ': ' . round($rating, 2);
        if ($rating) {
            while ($rating >= 2) {
     ...
     */
    public function fillMarkerArray(&$array, $record, $localConf, $categoryUID, $pageID = '')
    {
        $file = GeneralUtility::getFileAbsFileName($this->filePath . $record['file']);
        $fileInformation = $this->getTotalFileInfo($file);
        ...
        if ($teaseDownloads && $record['description'] != null) {
            $downloadteaser = $this->pi_RTEcssText(nl2br($this->local_cObj->stdWrap(
        ...
        if ($linkLabels != 'none') {
            if ($linkLabels == 'direct') {
                ...
                $downloadLabel = $this->pi_LinkTP(
                    htmlspecialchars(trim($record['label'])),
                    [],
                    false,
                    $this->filePath . $record['file']
                );
            }
            $array['###DOWNLOAD_LABEL###'] = $this->local_cObj->stdWrap(
                $downloadLabel,
        ...
        if ($this->pi_getFFvalue($this->flexform, 'iconsForLinks', 's_display')) {
            ...
            $downloadDetails = $this->pi_LinkTP($this->local_cObj->stdWrap(
                htmlspecialchars(trim($this->pi_getLL('ll_details'))),
                ''
            ), [
                'tx_abdownloads_pi1[action]' => 'getviewdetailsfordownload',
        ...
        if ($this->pi_getFFvalue(
            $this->flexform,
            'noDetailsInCategoryView',
            's_display'
        ) || $this->conf['noDetailsInCategoryView']) {
            $array['###DOWNLOAD_DETAILS###'] = '';
        } else {
            $array['###DOWNLOAD_DETAILS###'] = $this->local_cObj->stdWrap(
                $downloadDetails,
        ...
        if ($this->pi_getFFvalue(
            $this->flexform,
            'noStarsInCategoryView',
            's_display'
        ) || $this->conf['noStarsInCategoryView']) {
        ...
        if ($record['status'] == 2) {
            if ($this->pi_getFFvalue(
                $this->flexform,
                'noReportingInCategoryView',
                's_display'
            ) || $this->conf['noReportingInCategoryView']) {
            ...
            if ($this->pi_getFFvalue($this->flexform, 'iconsForLinks', 's_display')) {
                ...
                $download = $this->pi_LinkTP($this->local_cObj->stdWrap(
                    htmlspecialchars(trim($this->pi_getLL('ll_report_download_broken'))),
                    ''
                ), [
                    'tx_abdownloads_pi1[action]' => 'getviewreportbrokendownload',
        ...
        if ($this->pi_getFFvalue(
            $this->flexform,
            'noRatingInCategoryView',
            's_display'
        ) || $this->conf['noRatingInCategoryView']) {
            $array['###DOWNLOAD_RATE###'] = '';
        } else {
        ...
        // MISC
        $array['###DOWNLOAD_DESCRIPTION###'] = $this->pi_RTEcssText(nl2br($this->local_cObj->stdWrap(
            trim($record['description']),
            $localConf['downloadDescription_stdWrap.']
        )));
        $array['###DOWNLOAD_SPONSORED_DESCRIPTION###'] = $this->pi_RTEcssText(nl2br($this->local_cObj->stdWrap(
            trim($record['sponsored_description']),
            $localConf['downloadSponsoredDescription_stdWrap.']
        )));
        $array['###DOWNLOAD_LANGUAGE###'] = $this->local_cObj->stdWrap(
            $this->getLanguage($record['language_uid']),
            $localConf['downloadLanguage_stdWrap.']
        );
        $array['###DOWNLOAD_LICENSE###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($record['license'])),
            $localConf['downloadLicense_stdWrap.']
        );
        $array['###DOWNLOAD_HOMEPAGE###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($record['homepage'])),
        ...
        $array['###DOWNLOAD_SIZE###'] = $this->local_cObj->stdWrap(
            GeneralUtility::formatSize($fileInformation['size']) . 'Byte',
            ''
        );
        ...
        // LL VALUES
        $array['###LL_FILENAME###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_filename'))),
            ''
        );
        ...
        $array['###LL_DESCRIPTION###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_description'))),
            ''
        );
        $array['###LL_SPONSORED_DESCRIPTION###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_sponsored_description'))),
            ''
        );
        $array['###LL_LANGUAGE###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_language'))),
            ''
        );
        $array['###LL_LICENSE###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_license'))),
            ''
        );
        $array['###LL_HOMEPAGE###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_homepage'))),
            ''
        );
        $array['###LL_DATE_ADDED###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_added'))),
            ''
        );
        $array['###LL_CLICKS###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_clicks_lower'))),
            ''
        );
        $array['###LL_RATING###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_rating'))),
            ''
        );
        ...
        $array['###LL_DOWNLOAD###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_download'))),
            ''
        );
        $array['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap(
            htmlspecialchars(trim($this->pi_getLL('ll_downloads'))),
            $localConf['downloads_stdWrap.']
        );
        ...
        // EDIT PANEL
        $array['###EDIT_PANEL###'] = $this->pi_getEditPanel(
            $record,
            $this->tablePrefix . 'download',
            $record['uid'],
            $localConf
