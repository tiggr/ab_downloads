...
class tx_abdownloads_pi1 extends tslib_pibase {
	...
	function displayCategory( $categoryUID = 0 ) {
		...
		// Create marker array
		$markerArray = array();
		$markerArray['###CATEGORY_PATH###'] = $this->local_cObj->stdWrap( $this->getCategoryPath( $categoryUID ), $localConf['categoryPath_stdWrap.'] );

		// Substitute the markers in the given sub sub part
		$subpartContent = null;
		$subpartContent = $this->cObj->substituteMarkerArray( $this->cObj->getSubpart( $templateCode, '###' . $subSub_pathmenu. '###' ), $markerArray );

		// Substitute the template code with the given subpartcontent
		$templateCode = $this->cObj->substituteSubpart( $templateCode, '###' . $subSub_pathmenu . '###', $subpartContent );
		...
		if( $isLoggedIn == 1 || $allowAddDownloads == 1 ) {
			...
			$download = $this->pi_LinkTP( htmlspecialchars( trim( $this->pi_getLL( 'll_add_download' ) ) ), array( 'tx_abdownloads_pi1[action]' => 'getviewaddnewdownload', 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching );
			$markerArray['###DOWNLOAD_ADD_NEW###'] = $this->local_cObj->stdWrap( $download, $localConf['downloadAddNew_stdWrap.'] );
			$GLOBALS['TSFE']->ATagParams = $originalATagParams;

			// Substitute the markers in the given sub sub part
			$subpartContent = null;
			$subpartContent = $this->cObj->substituteMarkerArray( $this->cObj->getSubpart( $templateCode, '###' . $subSub_additional. '###' ), $markerArray );

			// Substitute the template code with the given subpartcontent
			$templateCode = $this->cObj->substituteSubpart( $templateCode, '###' . $subSub_additional . '###', $subpartContent );
		} else {
		...
		if( $categoryUID == 0 ) {
			...
			$orderBy = '';
			$limit = '';
			$downloadsInCurrentCategoryResults = $GLOBALS['TYPO3_DB']->exec_SELECTquery( '*', $databaseTable, $whereClause, $groupBy, $orderBy, $limit );
		} else {
			$databaseTable = $this->tablePrefix . 'download';
		...
		// Get record overlay
		$downloadsInCurrentCategory = $this->getRecordOverlay( $downloadsInCurrentCategoryResults, $this->tablePrefix . 'download' );
		...
		// Get category record(s)
		$categoryLabelsResults = $this->getCategoryRecords( 'parent_category', $categoryUID, $categorySortBy . ' ' . $categorySortOrder );
		...
		if( count( $categoryLabels ) > 0 ) {
			...
			for( $i = 0; $i < count( $categoryLabels ); $i++ ) {
				...
				$download = $this->pi_LinkTP( htmlspecialchars( trim( $categoryLabels[$i]['label'] ) ), array( 'tx_abdownloads_pi1[action]' => $action, 'tx_abdownloads_pi1[category_uid]' => $categoryLabels[$i]['uid'], 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching );
				$GLOBALS['TSFE']->ATagParams = $originalATagParams;

				// Create marker array
				$markerArrayCategory = array();

				$markerArrayCategory['###CATEGORY_ICON###'] = $this->getImageLink( $categoryLabels[$i], 'image', 'category', 'list', $categoryUID );
				$markerArrayCategory['###CATEGORY_LABEL###'] = $this->local_cObj->stdWrap( $download, $localConf['categoryLabel_stdWrap.'] );
				...
				// Substitute the markers in the given sub sub part
				$categoryList .= $this->cObj->substituteMarkerArray( $this->cObj->getSubpart( $templateCode, '###' . $subSubSub_category . '###' ), $markerArrayCategory );
			}

			// Prepare title array
			$markerArrayTitle = array();
			$markerArrayTitle['###LL_CATS###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_cats' ) ) ), $localConf['categories_stdWrap.'] );
			$markerArrayTitle['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_downloads_lower' ) ) ), $localConf['downloads_stdWrap.'] );
			...
			$subpartContent = $this->cObj->substituteMarkerArrayCached( $this->cObj->getSubpart( $templateCode, '###' . $subSub_categories . '###' ), $markerArrayTitle, $subpartArray, $wrappedSubpartArray );

			$templateCode = $this->cObj->substituteSubpart( $templateCode, '###' . $subSub_categories . '###', $subpartContent );
		} else {
		...
		$listLimit = intval( $this->pi_getFFvalue( $this->flexform, 'listLimit', 's_display' ) ? $this->pi_getFFvalue( $this->flexform, 'listLimit', 's_display' ) : $this->conf['listLimit'] );
		...
		if( $categoryUID == 0 ) {
			...
			$downloadsResults = $GLOBALS['TYPO3_DB']->exec_SELECTquery( '*', $databaseTable, $whereClause, $groupBy, $orderBy, $limit );
		} else {
			$databaseTable = $this->tablePrefix . 'download';
			$relationTable = $this->tablePrefix . 'category_mm';
			$foreignTable = $this->tablePrefix . 'category';
			...
			$orderBy = $downloadSortBy . ' ' . $downloadSortOrder;
			$limit = $limitStart . ', ' . $listLimit;
			$downloadsResults = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query( $databaseTable . '.*', $databaseTable, $relationTable, $foreignTable, ' AND ' . $theField . '=' . $GLOBALS['TYPO3_DB']->fullQuoteStr( $theValue, $databaseTable ) . ' ' . $whereClause, $groupBy, $orderBy, $limit );
		}
		...
		if( count( $downloads ) > 0 ) {
			...
			for( $i = 0; $i < count( $downloads ); $i++ ) {
				// Create marker array
				$markerArrayDownload = array();

				$this->fillMarkerArray( $markerArrayDownload, $downloads[$i], $localConf, $categoryUID );

				// Substitute the markers in the given sub sub part
				$downloadList .= $this->cObj->substituteMarkerArray( $this->cObj->getSubpart( $templateCode, '###' . $subSubSub_download . ( ( $i % $this->alternatingLayouts + 1 ) ? '_' . ( $i % $this->alternatingLayouts + 1 ) : '' ) . '###' ), $markerArrayDownload );
			}

			// Prepare title array
			$markerArrayTitle = array();
			$markerArrayTitle['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_downloads' ) ) ), $localConf['downloads_stdWrap.'] );

			$wrappedSubpartArray = array();
			$subpartArray['###DOWNLOAD###'] = $downloadList;
			$subpartContent = $this->cObj->substituteMarkerArrayCached( $this->cObj->getSubpart( $templateCode, '###' . $subSub_downloads . '###' ), $markerArrayTitle, $subpartArray, $wrappedSubpartArray);
		...
		if( count( $downloadsInCurrentCategory ) > $listLimit && !$this->conf['noPageBrowser'] && !$this->pi_getFFvalue( $this->flexform, 'noPageBrowser', 's_display' ) ) {
			...
			if( $this->conf['userPageBrowserFunc'] ) {
				...
				if( $this->conf['usePiBasePagebrowser'] ) {
					...
					$markerArray['###BROWSE_DOWNLOADS###'] = $this->pi_list_browseresults( $this->conf['pageBrowser.']['showResultCount'], $this->conf['pageBrowser.']['tableParams'], $wrapArr, $pointerName, $this->conf['pageBrowser.']['hscText'] );
				} else {
	...
	function displayTree( $categoryUID = 0, $level = 0 )	 {
		...
		if( count( $downloads ) > 0 ) {
			for( $j = 0; $j < count( $downloads ); $j++ ) {
				...
				// Substitute the markers in the given sub sub part
				$downloadList .= $this->cObj->substituteMarkerArray( $this->cObj->getSubpart( $templateCode, '###' . $subSub_treedownload . '###' ), $markerArray );
			}
			...
			$markerArrayMessage['###CATEGORY_ICON###'] = $this->getImageLink( $category[0], 'image', 'category', 'tree', $categoryUID );
			$markerArrayMessage['###CATEGORY_LABEL###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $category[0]['label'] ) ), $localConf['categoryLabel_stdWrap.'] );

			if( $isLoggedIn == 1 || $allowAddDownloads == 1 ) {
				$GLOBALS['TSFE']->ATagParams = $GLOBALS['TSFE']->ATagParams . ( $localConf['downloadAddNew.']['ATagParams'] ? ' ' . $localConf['downloadAddNew.']['ATagParams'] : '' );
				$download = $this->pi_LinkTP( htmlspecialchars( trim( $this->pi_getLL( 'll_add_download') ) ), array( 'tx_abdownloads_pi1[action]' => 'getviewaddnewdownload', 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching );
				$markerArrayMessage['###DOWNLOAD_ADD_NEW###'] = $this->local_cObj->stdWrap( $download, $localConf['downloadAddNew_stdWrap.'] );
				$GLOBALS['TSFE']->ATagParams = $originalATagParams;
			} else {
		...
		// Get subcategories
		$subcategoriesResults = $this->getCategoryRecords( 'parent_category', $categoryUID, $categorySortBy . ' ' . $categorySortOrder );
	...
	function displaySearch() {
		...
		if( $searchWords != null ) {
			...
			for( $i = 0; $i < count( $downloads ); $i++ ) {
				...
				if( $categoryUID != '0' ) {
					$GLOBALS['TSFE']->ATagParams = $GLOBALS['TSFE']->ATagParams . ( $localConf['categoryLabel.']['ATagParams'] ? ' ' . $localConf['categoryLabel.']['ATagParams'] : '' );
					$download = $this->pi_LinkTP( htmlspecialchars( trim( $category[0]['label'] ) ), array( 'tx_abdownloads_pi1[action]' => 'getviewcategory', 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching );
					$GLOBALS['TSFE']->ATagParams = $originalATagParams;
				}
				...
				$markerArray['###CATEGORY_PATH###'] = $this->local_cObj->stdWrap( $this->getCategoryPath( $categoryUID ), $localConf['categoryPath_stdWrap.'] );
				$markerArray['###CATEGORY_ICON###'] = $this->getImageLink( $category[0], 'image', 'category', 'search', $categoryUID );
				...
				$pageID = intval( $this->pi_getFFvalue( $this->flexform, 'pageListPlugin', 'sDEF' ) ? $this->pi_getFFvalue( $this->flexform, 'pageListPlugin', 'sDEF' ) : $GLOBALS['TSFE']->id );

				$this->fillMarkerArray( $markerArray, $downloads[$i], $localConf, $categoryUID, $pageID );

				// Substitute the markers in the given sub sub part
				$subpartContent .= $this->cObj->substituteMarkerArray( $this->cObj->getSubpart( $templateCode, '###' . $subSub_downloads . '###' ), $markerArray );
			}
			...
			if( count( $downloads ) != 0 ) {
				...
				$markerArray['###NO_DOWNLOADS_MESSAGE###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'no_downloads_message' ) ) ), $localConf['noDownloadsMessage_stdWrap.'] );

				// Substitute the markers in the given sub sub part
				$subpartContent = $this->cObj->substituteMarkerArray( $this->cObj->getSubpart( $templateCode, '###' . $subSub_nodownloads . '###' ), $markerArray );

				// Substitute the sub sub part markers with the given subpartcontents.
				$templateCode = $this->cObj->substituteSubpart( $templateCode, '###' . $subSub_nodownloads . '###', $subpartContent );
				$templateCode = $this->cObj->substituteSubpart( $templateCode, '###' . $subSub_downloads . '###', '' );
			}
		} else {
		...
		$markerArray['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_downloads' ) ) ), $localConf['downloads_stdWrap.'] );
		...
		if( count( $downloads ) > $listLimit && !$this->conf['noPageBrowser'] && !$this->pi_getFFvalue( $this->flexform, 'noPageBrowser', 's_display' ) ) {
			//  configure pagebrowser vars
...
*/
					...
					$topDownloadsResults = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query( $selectExtension . $databaseTable . '.*', $databaseTable, $relationTable, $foreignTable, ' AND ' . $whereClause, $groupBy, $orderBy, $limit );
					...
					for( $i = 0; $i < count( $topDownloads ); $i++ ) {
						...
						$this->fillMarkerArray( $markerArrayDownload, $topDownloads[$i], $localConf, $categoryUID, $pageID );
						...
						// Substitute the markers in the given sub sub part
						$downloadList .= $this->cObj->substituteMarkerArray( $this->cObj->getSubpart( $templateCode, '###' . $subSub_topdownload . '###' ), $markerArrayDownload );
					}
	 ...
	 * @param	integer		$level	Level in the catalog.
 	...
 	function displayCatalog( $categoryUID = 0, $level = 0 ) {
		...
		$categorySortBy = $this->pi_getFFvalue( $this->flexform, 'categorySortBy', 's_display' );
		$categorySortOrder = $this->pi_getFFvalue( $this->flexform, 'categorySortOrder', 's_display' );
		$numberOfColumns = intval( $this->pi_getFFvalue( $this->flexform, 'numColumns', 's_catalog' ) ? $this->pi_getFFvalue( $this->flexform, 'numColumns', 's_catalog' ) : 2 );
		$maxNumSubcat = intval( $this->pi_getFFvalue( $this->flexform, 'maxSubcategories', 's_catalog' ) ? $this->pi_getFFvalue( $this->flexform, 'maxSubcategories', 's_catalog' ) : 2 );
		...
		if( count( $categoryLabels ) > 0 ) {
			...
			for( $i = 0; $i < count( $categoryLabels ); $i++ ) {
				...
				$markerArrayCategory['###CATEGORY_ICON###'] = $this->getImageLink( $categoryLabels[$i], 'image', 'category', 'catalog', $categoryUID );
				$markerArrayCategory['###CATEGORY_LABEL###'] = $this->local_cObj->stdWrap( $download, $localConf['categoryLabel_stdWrap.'] );
				$markerArrayCategory['###CATEGORY_DOWNLOAD_COUNT###'] = $this->local_cObj->stdWrap( $categoryDownloadCount, '' );
				$markerArrayCategory['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_downloads_lower' ) ) ), '' );
				$markerArrayCategory['###LL_TOTAL###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_total' ) ) ), '' );
				...
				// Get subcategories
				$subcategoryLabelsResults = $this->getCategoryRecords( 'parent_category', $categoryLabels[$i]['uid'], $categorySortBy . ' ' . $categorySortOrder, $maxNumSubcat );

				// Get record overlay
				$subcategoryLabels = $this->getRecordOverlay( $subcategoryLabelsResults, $this->tablePrefix . 'category' );
				...
				if( count( $subcategoryLabels ) > 0 ) {
					for( $j = 0; $j < count( $subcategoryLabels ); $j++ ) {
						...
						$download = $this->pi_LinkTP( htmlspecialchars( trim( $subcategoryLabels[$j]['label'] ) ), array( 'tx_abdownloads_pi1[action]' => $action, 'tx_abdownloads_pi1[category_uid]' => $subcategoryLabels[$j]['uid'], 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching );
						$GLOBALS['TSFE']->ATagParams = $originalATagParams;

						$markerArraySubcategory['###SUBCATEGORY_ICON###'] = $this->getImageLink( $subcategoryLabels[$j], 'image', 'category', 'catalog', $categoryUID );
						$markerArraySubcategory['###SUBCATEGORY_LABEL###'] = $this->local_cObj->stdWrap( $download, $localConf['subcategoryLabel_stdWrap.'] );
						...
						// Substitute the markers in the given sub sub part
						$subcategoryList .= $this->cObj->substituteMarkerArray( $this->cObj->getSubpart( $templateCode, '###' . $subSubSub_subcategories . '###' ), $markerArraySubcategory ) . $separator;
					}
				}
				...
				$subpartContent .= $this->cObj->substituteMarkerArrayCached( $this->cObj->getSubpart( $templateCode, '###' . $subSub_category . '###' ), $markerArrayCategory, $subpartArray, $wrappedSubpartArray );
			}

			$templateCode = $this->cObj->substituteSubpart( $templateCode, '###' . $subSub_category . '###', $subpartContent );
		} else {
	...
	function getViewDetailsForDownload( $uid = null, $categoryUID ) {
		...
		if( $uid != null ) {
			...
			if( is_array( $download ) && $download[0]['pid'] > 0 ) {
				...
				$markerArray['###CATEGORY_PATH###'] = $this->local_cObj->stdWrap( $this->getCategoryPath( $categoryUID ), $this->conf['categoryPath_stdWrap.'] );
				// FIXME: Add specific detailed download view downloadImage.ATagParams
				$markerArray['###DOWNLOAD_IMAGE###'] = $this->getImageLink( $download[0], 'image', 'link', 'list', $categoryUID, true );
				...
				$templateCode = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'non_public_download_message' ) ) ), $this->conf['nonPublicDownloadMessage_stdWrap.'] );
			}
		} else {
			die( 'ERROR: No download UID given!' );
		}
	...
	function getViewAddNewDownload( $categoryUID, $form_errormsg = null ) {
		...
		$possibleFields = array( 'label', 'category', 'contact', 'image', 'file', 'description' );
		$mandatoryFields = $this->pi_getFFvalue( $this->flexform, 'mandatoryFields', 'sDEF' );

		// Get the maximum filesize for downloads
		$maxFileSize = $this->pi_getFFvalue( $this->flexform, 'maxFileSize', 'sDEF' ) ? $this->pi_getFFvalue( $this->flexform, 'maxFileSize', 'sDEF' ) : $this->conf['maxFileSize'];
		...
		$form_action = $this->pi_getPageLink( $GLOBALS['TSFE']->id, '_self', array( 'tx_abdownloads_pi1[action]' => 'getviewaddnewdownload', 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'no_cache' => '1' ) );
		...
		$markerArray['###FORM_ERRORMSG###'] = $this->local_cObj->stdWrap( $form_errormsg, $this->conf['formErrorMsg_stdWrap.'] );
		$markerArray['###LINK_BACK_TO_CATEGORY###'] = '<a href="javascript:history.back()">' . htmlspecialchars( trim( $this->pi_getLL( 'll_back' ) ) ) . '</a>';
		...
		$markerArray['###LL_ADD_DOWNLOAD###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_add_download' ) ) ), $this->conf['addDownload_stdWrap.'] );
		$markerArray['###LL_ADD_DOWNLOAD_TEXT###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_add_download_text' ) ) ), $this->conf['addDownloadText_stdWrap.'] );
		...
		foreach( $possibleFields as $id => $possibleField ) {
			...
			$markerArray['###LL_' . strtoupper( $possibleField ) . '###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_' . $possibleField ) ) ), $wrap );
			$wrap = '';
		}
	...
	function getViewAddNewDownloadResult( $categoryUID = 0 ) {
		...
		$allowAddDownloads = $this->pi_getFFvalue( $this->flexform, 'allowAddDownloads', 'sDEF' ) ? $this->pi_getFFvalue( $this->flexform, 'allowAddDownloads', 'sDEF' ) : $this->conf['allowAddDownloads'];
		$statusAddedDownloads = $this->pi_getFFvalue( $this->flexform, 'statusAddedDownloads', 'sDEF' ) ? $this->pi_getFFvalue( $this->flexform, 'statusAddedDownloads', 'sDEF' ) : $this->conf['statusAddedDownloads'];
		...
		$subject = $this->pi_getFFvalue( $this->flexform, 'emailSubjectAdd', 's_notification' ) ? $this->pi_getFFvalue( $this->flexform, 'emailSubjectAdd', 's_notification' ) : $this->conf['emailSubjectAdd'];
	...
	function getViewReportBrokenDownload( $uid = null, $categoryUID ) {
		...
		if( $uid != null ) {
			...
			// Generate values for marker array
			$form_action = $this->pi_getPageLink( $GLOBALS['TSFE']->id, '_self', array( 'tx_abdownloads_pi1[action]' => 'getviewreportbrokendownload', 'tx_abdownloads_pi1[uid]' => $uid, 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'no_cache' => '1' ) );
	...
	function getViewReportBrokenDownloadResult( $uid = null, $categoryUID ) {
		...
		if( $uid != null ) {
			...
			if( $download[0]['status'] == 1 ) {
				...
				$name = $this->pi_getFFvalue( $this->flexform, 'adminName', 's_notification' ) ? $this->pi_getFFvalue( $this->flexform, 'adminName', 's_notification' ) : $this->conf['adminName'];
				$email = $this->pi_getFFvalue( $this->flexform, 'adminEmail', 's_notification' ) ? $this->pi_getFFvalue( $this->flexform, 'adminEmail', 's_notification' ) : $this->conf['adminEmail'];
				$subject = $this->pi_getFFvalue( $this->flexform, 'emailSubjectBroken', 's_notification' ) ? $this->pi_getFFvalue( $this->flexform, 'emailSubjectBroken', 's_notification' ) : $this->conf['emailSubjectBroken'];
				$ll_label = htmlspecialchars( trim( $this->pi_getLL( 'll_label' ) ) );
				$ll_description = htmlspecialchars( trim( $this->pi_getLL( 'll_description' ) ) );
				$ll_category = htmlspecialchars( trim( $this->pi_getLL( 'll_category' ) ) );
	...
	function getViewRateDownload( $uid = null, $categoryUID ) {
		...
		if( $uid != null ) {
			...
			$markerArray['###LL_RATE_DOWNLOAD_TEXT###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_rate_download_text' ) ) ), $this->conf['rateDownloadText_stdWrap.'] );
	...
	function makePageBrowser( $pointerName = 'pointer' ) {
		...
		if( $this->pi_alwaysPrev >= 0 ) {
			if( $pointer > 0 ) {
				$links[] = '<td nowrap="nowrap"><p>' . $this->pi_linkTP_keepPIvars( htmlspecialchars( trim( $this->pi_getLL( 'pi_list_browseresults_prev', '< Previous' ) ) ), array( 'tx_abdownloads_pi1[action]' => $action, 'tx_abdownloads_pi1[category_uid]' => $categoryUID, $pointerName => ( $pointer - 1 ? $pointer - 1 : '' ) ), $this->allowCaching ) . '</p></td>';
			} elseif( $this->pi_alwaysPrev ) {
				$links[] = '<td nowrap="nowrap"><p>' . htmlspecialchars( trim( $this->pi_getLL( 'pi_list_browseresults_prev', '< Previous' ) ) ) . '</p></td>';
			}
		}

		for( $a= 0; $a < $max; $a++ ) {
			$links[] = '<td' .( $pointer == $a ? $this->pi_classParam( 'browsebox-SCell' ) : '' ) . ' nowrap="nowrap"><p>' . $this->pi_linkTP_keepPIvars(trim( htmlspecialchars( trim( $this->pi_getLL( 'pi_list_browseresults_page', 'Page' ) ) ) . ' ' . ( $a + 1) ), array( 'tx_abdownloads_pi1[action]' => $action, 'tx_abdownloads_pi1[category_uid]' => $categoryUID, $pointerName => ( $a ? $a : '' ) ), $this->allowCaching ) . '</p></td>';
		}

		if( $pointer < ceil( $count/$results_at_a_time ) - 1 ) {
			$links[] = '<td nowrap="nowrap"><p>' . $this->pi_linkTP_keepPIvars( htmlspecialchars( trim( $this->pi_getLL( 'pi_list_browseresults_next', 'Next >' ) ) ), array( 'tx_abdownloads_pi1[action]' => $action, 'tx_abdownloads_pi1[category_uid]' => $categoryUID, $pointerName => $pointer + 1), $this->allowCaching ) . '</p></td>';
		}
		...
		$sTables = '
			...
			<div' . $this->pi_classParam( 'browsebox' ) . '>' . ( $showResultCount ? '<p>' . ( $this->internal['res_count'] ?
				sprintf( str_replace( '###SPAN_BEGIN###', '<span' . $this->pi_classParam( 'browsebox-strong' ) . '>', htmlspecialchars( trim( $this->pi_getLL( 'pi_list_browseresults_displays', 'Displaying results ###SPAN_BEGIN###%s to %s</span> out of ###SPAN_BEGIN###%s</span>' ) ) ) ),
				$this->internal['res_count'] > 0 ? $pR1 : 0, min(array( $this->internal['res_count'], $pR2) ), $this->internal['res_count'] ) :
				htmlspecialchars( trim( $this->pi_getLL( 'pi_list_browseresults_noResults', 'Sorry, no items were found. ' ) ) ) ) . '</p>' : '' ) . '
	...
	function initPidList() {
		...
		$recursive = is_numeric( $recursive ) ? $recursive : $this->cObj->stdWrap( $this->conf['recursive'], $this->conf['recursive.'] );
	...
	function recursiveDownloadCount( $categoryUID ) {
		...
		$orderBy = '';
		$limit = '';
		$downloadsInCurrentCategoryResults = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query( $databaseTable . '.*', $databaseTable, $relationTable, $foreignTable, $whereClause, $groupBy, $orderBy, $limit );
	...
	function getCategoryPath( $categoryUID, $showLinks = true ) {
		...
		if( $categoryUID != '0' ) {
			...
			if( $showLinks ) {
				$content = $this->pi_LinkTP( htmlspecialchars( trim( $this->pi_getLL( 'll_home' ) ) ), array( 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching, $pageID ) . $content;
			} else {
		...
		if( $resultSet != null && $databaseTable != null ) {
			...
			while ( $row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc( $resultSet ) ) {
				...
				if ( $GLOBALS['TSFE']->sys_language_content ) {
					...
					$row = $GLOBALS['TSFE']->sys_page->getRecordOverlay( $databaseTable, $row, $GLOBALS['TSFE']->sys_language_content, $OLmode );
				}
	...
	function getCategorySelect( $categoryUID, $selectedID = 0 ) {
		...
		if( $categoryUID != '0' ) {
			...
			$content .= '<option value="' . $categoryUID . '">' . $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_cats' ) ) ), '' ) . '</option>';
		}
	...
	function getDownloadRecord( $downloadUID ) {
		...
		$orderBy = '';
		$limit = '';
		$downloadResult = $GLOBALS['TYPO3_DB']->exec_SELECTquery( '*', $databaseTable, $theField . '=' . $GLOBALS['TYPO3_DB']->quoteStr( $theValue, $databaseTable ) . ' ' . $whereClause, $groupBy, $orderBy, $limit );
	...
	function getCategoryRecords( $field, $categoryUID, $orderBy = '', $limit = '' ) {
		...
		$groupBy = '';
		$categoryResult = $GLOBALS['TYPO3_DB']->exec_SELECTquery( '*', $databaseTable, $field . '=' . $GLOBALS['TYPO3_DB']->quoteStr( $theValue, $databaseTable ) . ' ' . $whereClause, $groupBy, $orderBy, $limit );
	...
	function getCategoryUID( $recordUID ) {
		...
		$orderBy = 'sorting ASC';
		$limit = '';
		$categoriesResult = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query( $foreignTable . '.*', $databaseTable, $relationTable, $foreignTable, ' AND ' . $theField . '=' . $GLOBALS['TYPO3_DB']->fullQuoteStr( $theValue, $databaseTable ) . ' ' . $whereClause, $groupBy, $orderBy, $limit );
		...
		switch( (string)$type ) {
			case 'link':
				if( $record[$field] ) {
					...
					if( $detailedView ) {
						$imageMaxHeight = intval( $this->pi_getFFvalue( $this->flexform, 'imageMaxHeightDetailed', 's_image' ) ? $this->pi_getFFvalue( $this->flexform, 'imageMaxHeightDetailed', 's_image' ) : $this->conf['imageMaxHeightDetailed'] );
						$imageMaxWidth = intval( $this->pi_getFFvalue( $this->flexform, 'imageMaxWidthDetailed', 's_image' ) ? $this->pi_getFFvalue( $this->flexform, 'imageMaxWidthDetailed', 's_image' ) : $this->conf['imageMaxWidthDetailed'] );
					} else {
						$imageMaxHeight = intval( $this->pi_getFFvalue( $this->flexform, 'imageMaxHeightCategory', 's_image' ) ? $this->pi_getFFvalue( $this->flexform, 'imageMaxHeightCategory', 's_image' ) : $this->conf['imageMaxHeightCategory'] );
						$imageMaxWidth = intval( $this->pi_getFFvalue( $this->flexform, 'imageMaxWidthCategory', 's_image' ) ? $this->pi_getFFvalue( $this->flexform, 'imageMaxWidthCategory', 's_image' ) : $this->conf['imageMaxWidthCategory'] );
					}
	...
	function returnStarsForRating( $rating ) {
		...
		$iconText = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_rating' ) ) ), '' ) . ': ' . round( $rating, 2 );
		if( $rating ) {
			while( $rating >= 2 ) {
				$rating -= 2;
				$count += 1;
	...
	function fillMarkerArray( &$array, $record, $localConf, $categoryUID, $pageID = '' ) {
		...
		if( $linkLabels != 'none' ) {
			if( $linkLabels == 'direct' ) {
				$downloadLabel = $this->pi_LinkTP( htmlspecialchars( trim( $record['label'] ) ), array( 'tx_abdownloads_pi1[action]' => 'getviewclickeddownload', 'tx_abdownloads_pi1[uid]' => $record['uid'], 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), false, $pageID );
			} elseif( $linkLabels == 'details' ) {
				$downloadLabel = $this->pi_LinkTP( htmlspecialchars( trim( $record['label'] ) ), array( 'tx_abdownloads_pi1[action]' => 'getviewdetailsfordownload', 'tx_abdownloads_pi1[uid]' => $record['uid'], 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching, $pageID );
			} elseif( $linkLabels == 'target' ) {
				$downloadLabel = $this->pi_LinkTP( htmlspecialchars( trim( $record['label'] ) ), array(), false, $this->filePath . $record['file'] );
			}
			$array['###DOWNLOAD_LABEL###'] = $this->local_cObj->stdWrap( $downloadLabel, $localConf['downloadLabel_stdWrap.'] );
		} else {
			$array['###DOWNLOAD_LABEL###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $record['label'] ) ), $localConf['downloadLabel_stdWrap.'] );
		}
		$GLOBALS['TSFE']->ATagParams = $originalATagParams;
		...
		if( $this->pi_getFFvalue( $this->flexform, 'iconsForLinks', 's_display' ) ) {
			$downloadDetails = $this->pi_LinkTP( $this->local_cObj->fileResource( $this->conf['iconDetails'], 'alt="' . htmlspecialchars( trim( $this->pi_getLL( 'll_details' ) ) ) . '" title="' . htmlspecialchars( trim( $this->pi_getLL( 'll_details' ) ) ) . '"' ), array( 'tx_abdownloads_pi1[action]' => 'getviewdetailsfordownload', 'tx_abdownloads_pi1[uid]' => $record['uid'], 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching, $pageID );
		} else {
			$downloadDetails = $this->pi_LinkTP( $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_details' ) ) ), '' ), array( 'tx_abdownloads_pi1[action]' => 'getviewdetailsfordownload', 'tx_abdownloads_pi1[uid]' => $record['uid'], 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching, $pageID );
		}

		if( $this->pi_getFFvalue( $this->flexform, 'noDetailsInCategoryView', 's_display' ) || $this->conf['noDetailsInCategoryView'] ) {
			$array['###DOWNLOAD_DETAILS###'] = '';
		} else {
			$array['###DOWNLOAD_DETAILS###'] = $this->local_cObj->stdWrap( $downloadDetails, $localConf['downloadDetails_stdWrap.'] );
		}
		$GLOBALS['TSFE']->ATagParams = $originalATagParams;

		// RATING_STARS
		if( $this->pi_getFFvalue( $this->flexform, 'noStarsInCategoryView', 's_display' ) || $this->conf['noStarsInCategoryView'] ) {
		...
		// REPORT_BROKEN
		if( $record['status'] == 2 ) {
			if( $this->pi_getFFvalue( $this->flexform, 'noReportingInCategoryView', 's_display' ) || $this->conf['noReportingInCategoryView'] ) {
				$array['###DOWNLOAD_REPORT_BROKEN###'] = '';
			} else {
				if( $this->pi_getFFvalue( $this->flexform, 'iconsForLinks', 's_display' ) ) {
					$array['###DOWNLOAD_REPORT_BROKEN###'] = $this->local_cObj->stdWrap( $this->local_cObj->fileResource( $this->conf['iconReportBrokenDisabled'], 'alt="' . htmlspecialchars( trim( $this->pi_getLL( 'll_reported_download_broken' ) ) ) . '" title="' . htmlspecialchars( trim( $this->pi_getLL( 'll_reported_download_broken' ) ) ) . '"' ), $localConf['downloadReportBroken_stdWrap.'] );
				} else {
					$array['###DOWNLOAD_REPORT_BROKEN###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_reported_download_broken') ) ), $localConf['downloadReportBroken_stdWrap.'] );
				}
			}
		} else {
			...
			if( $this->pi_getFFvalue( $this->flexform, 'iconsForLinks', 's_display' ) ) {
				$download = $this->pi_LinkTP( $this->local_cObj->fileResource( $this->conf['iconReportBroken'], 'alt="' . htmlspecialchars( trim( $this->pi_getLL( 'll_report_download_broken' ) ) ) . '" title="' . htmlspecialchars( trim( $this->pi_getLL( 'll_report_download_broken' ) ) ) . '"' ), array( 'tx_abdownloads_pi1[action]' => 'getviewreportbrokendownload', 'tx_abdownloads_pi1[uid]' => $record['uid'], 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching, $pageID );
			} else {
				$download = $this->pi_LinkTP( $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_report_download_broken') ) ), '' ), array( 'tx_abdownloads_pi1[action]' => 'getviewreportbrokendownload', 'tx_abdownloads_pi1[uid]' => $record['uid'], 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching, $pageID );
			}

			if( $this->pi_getFFvalue( $this->flexform, 'noReportingInCategoryView', 's_display' ) || $this->conf['noReportingInCategoryView'] ) {
				$array['###DOWNLOAD_REPORT_BROKEN###'] = '';
			} else {
				$array['###DOWNLOAD_REPORT_BROKEN###'] = $this->local_cObj->stdWrap( $download, $localConf['downloadReportBroken_stdWrap.'] );
			}
			$GLOBALS['TSFE']->ATagParams = $originalATagParams;
		}

		// RATE
		if( $this->pi_getFFvalue( $this->flexform, 'noRatingInCategoryView', 's_display' ) || $this->conf['noRatingInCategoryView'] ) {
			$array['###DOWNLOAD_RATE###'] = '';
		} else {
			$GLOBALS['TSFE']->ATagParams = $GLOBALS['TSFE']->ATagParams . ( $localConf['downloadRate.']['ATagParams'] ? ' ' . $localConf['downloadRate.']['ATagParams'] : '' );
			if( $this->pi_getFFvalue( $this->flexform, 'iconsForLinks', 's_display' ) ) {
				$download = $this->pi_LinkTP( $this->local_cObj->fileResource( $this->conf['iconRate'], 'alt="' . htmlspecialchars( trim( $this->pi_getLL( 'll_rate_download' ) ) ) . '" title="' . htmlspecialchars( trim( $this->pi_getLL( 'll_rate_download' ) ) ) . '"' ), array( 'tx_abdownloads_pi1[action]' => 'getviewratedownload', 'tx_abdownloads_pi1[uid]' => $record['uid'], 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching, $pageID );
			} else {
				$download = $this->pi_LinkTP( $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_rate_download') ) ), '' ), array( 'tx_abdownloads_pi1[action]' => 'getviewratedownload', 'tx_abdownloads_pi1[uid]' => $record['uid'], 'tx_abdownloads_pi1[category_uid]' => $categoryUID, 'tx_abdownloads_pi1[cid]' => $this->cObj->data['uid'] ), $this->allowCaching, $pageID );
			}
		...
		// ICON and IMAGE
		$array['###DOWNLOAD_ICON###'] = $this->local_cObj->fileResource( $this->conf['iconDownload'], 'alt="Download Icon" title="Download Icon"' );
		...
		$array['###DOWNLOAD_HREF###'] = $this->pi_getPageLink( $GLOBALS['TSFE']->id, '', array( 'tx_abdownloads_pi1[action]' => 'getviewclickeddownload', 'tx_abdownloads_pi1[uid]' => $record['uid'], 'no_cache' => '1' ) );
		...
		$array['###LL_DOWNLOADS###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_downloads' ) ) ), $localConf['downloads_stdWrap.'] );
		$array['###LL_TOTAL###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_total' ) ) ), '' );
		$array['###LL_CLICKS###'] = $this->local_cObj->stdWrap( htmlspecialchars( trim( $this->pi_getLL( 'll_clicks_lower' ) ) ), '' );

		// EDIT PANEL
		$array['###EDIT_PANEL###'] = $this->pi_getEditPanel( $record, $this->tablePrefix . 'download', $record['uid'], $localConf );
